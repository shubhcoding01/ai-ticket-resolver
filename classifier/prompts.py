...
SUPPORT_NAME   = os.getenv("SUPPORT_NAME",   "IT Support Team")
COMPANY_DOMAIN = os.getenv("COMPANY_DOMAIN", "icici.com")
SUPPORT_EMAIL  = os.getenv("SUPPORT_EMAIL",  f"itsupport@{os.getenv('COMPANY_DOMAIN', 'icici.com')}")
...
def _now_str(fmt: str = "%d %b %Y %H:%M UTC") -> str:
    """
...
def build_classification_prompt(
    ...
    """
    Build the user message for standard ticket classification.
    Fills CLASSIFICATION_PROMPT_TEMPLATE with ticket data.
    Uses timezone-aware UTC timestamp for submitted_at.
...
def build_reclassification_prompt(
    ...
    """
    Build prompt for re-classifying a ticket that was flagged
    ...
    Args:
        ...
        prev_auto       : Original can_auto_resolve value
        prev_confidence : Original confidence level
        flag_reason     : Why this ticket was flagged

    Returns:
        Formatted reclassification prompt string
    """
    desc_trimmed = (
        description[:1500]
        if len(description) > 1500
...
def build_sentiment_prompt(
    ...
    """
    Build prompt for analyzing user sentiment in a ticket.
    Used to detect frustrated or angry users for priority boost.
...
def build_quality_check_prompt(
    ...
    """
    Build prompt for assessing ticket completeness and quality.
    Used to detect tickets that need more info before processing.
...
def build_auto_reply_prompt(
    ...
    """
    Build prompt for generating personalized auto-reply emails.
    Used by notifier.py for richer AI-generated email content.
...
def get_system_prompt() -> str:
    """
    Return the system prompt for all Claude API calls.
    Called by ai_classifier.py when building API requests.
...
def get_category_description(category: str) -> str:
    ...
    Used in engineer escalation emails, dashboard labels,
    and resolution notification messages.
...
def get_estimated_resolution_time(
    ...
    """
    Return a human-readable estimated resolution time based
    on category and priority. Used in auto-reply emails to
    set user expectations.

    Logic:
        base_time (minutes) × priority_multiplier → minutes
        then bucketed into readable time strings.
    ...
    Returns:
        Human-readable time estimate string
        e.g. 'within 30 minutes', 'within 1-2 hours'
    """
    base_times = {
        "app_install"       : 30,
        "antivirus"         : 20,
...
if __name__ == "__main__":
    ...
    print(f"  Support name : {SUPPORT_NAME}")
    ...
    cp = build_classification_prompt(
        ...
        description    = (
            ...
            "Machine is PC-ICICI-0042. Please help."
        ),
        ticket_id      = 1001,
        machine_name   = "PC-ICICI-0042",
        requester_name = "Rahul Sharma",
    ...
    print("--- Batch Classification Prompt ---")
    tickets = [
        {
            "id"          : 1001,
            "subject"     : "Install Zoom urgently",
        ...
        },
        {
            "id"          : 1003,
            "subject"     : "Cannot login — locked out",
            "description" : "Forgot password, locked after 5 attempts.",
        },
    ]
    bp = build_batch_classification_prompt(tickets)
    print(bp)
    ...
    sp2 = build_sentiment_prompt(
        ...
        description = (
            "I have been trying to fix this since morning! "
            "Nothing works and I have a client deadline in 1 hour. "
            "This is completely unacceptable. Help me RIGHT NOW."
        ),
    )
    print(sp2)
    ...
    print("--- Reclassification Prompt ---")
    rp = build_reclassification_prompt(
        subject         = "Laptop screen flickering",
        description     = "My screen keeps flickering.",
        prev_category   = "os_issue",
        prev_priority   = "medium",
        prev_auto       = True,
        prev_confidence = "low",
        flag_reason     = "Rule classifier detected 'hardware'.",
    )
    print(rp[:300] + "...\n")
    ...
    for cat in [
        ...
        "hardware", "network", "password_reset",
    ]:
        for pri in ["urgent", "high", "medium", "low"]:
            t = get_estimated_resolution_time(cat, pri)
            print(
