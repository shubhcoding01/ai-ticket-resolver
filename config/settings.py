...
def _get_env(
    ...
    if required and not value:
        log.warning(
            f"Required setting '{key}' is not set in .env. "
            f"Using default: '{default}'"
        )
    return value
...
AUTOMATION_ENABLED = _get_bool("AUTOMATION_ENABLED", True)
POWERSHELL_PATH    = _get_env("POWERSHELL_PATH", "powershell.exe")
SCRIPTS_DIR        = str(BASE_DIR / "automation" / "scripts")
SCRIPT_TIMEOUT_SEC = _get_int("SCRIPT_TIMEOUT_SEC", 120)
MAX_RETRY_ATTEMPTS = _get_int("MAX_RETRY_ATTEMPTS",   2)
...
FEATURE_AUTO_RESOLVE         = _get_bool("FEATURE_AUTO_RESOLVE",         True)
FEATURE_KB_SEARCH            = _get_bool("FEATURE_KB_SEARCH",            True)
FEATURE_SENTIMENT_ANALYSIS   = _get_bool("FEATURE_SENTIMENT_ANALYSIS",   False)
FEATURE_TICKET_QUALITY_CHECK = _get_bool("FEATURE_TICKET_QUALITY_CHECK", False)
FEATURE_BATCH_PROCESSING     = _get_bool("FEATURE_BATCH_PROCESSING",     False)
FEATURE_AFTER_HOURS_REPLY    = _get_bool("FEATURE_AFTER_HOURS_REPLY",    True)
FEATURE_INTUNE_INTEGRATION   = _get_bool("FEATURE_INTUNE_INTEGRATION",   False)
FEATURE_DASHBOARD            = _get_bool("FEATURE_DASHBOARD",            True)
...
RATE_LIMIT_CLAUDE_RPM    = _get_int("RATE_LIMIT_CLAUDE_RPM",    50)
RATE_LIMIT_FRESHDESK_RPM = _get_int("RATE_LIMIT_FRESHDESK_RPM", 100)
RATE_LIMIT_SMTP_PER_HOUR = _get_int("RATE_LIMIT_SMTP_PER_HOUR", 100)
RATE_LIMIT_INTUNE_RPM    = _get_int("RATE_LIMIT_INTUNE_RPM",     60)
...
def validate_settings() -> dict:
    ...
    In demo mode relaxes the required API key checks since
    demo keys are intentionally placeholder values.
    Called at startup from main.py to catch missing config
    before the system starts processing tickets.

    Returns:
        Dict with keys:
            valid    : True if all required settings present
            errors   : List of missing/invalid setting descriptions
            warnings : List of optional settings that are not set
    """
    errors   = []
    ...
    if not DEMO_MODE:
        if (
            not FRESHDESK_DOMAIN
            or FRESHDESK_DOMAIN in demo_domain_values
        ):
            errors.append(
                "Missing required setting: FRESHDESK_DOMAIN "
                "(Freshdesk domain)"
            )

        if (
            not FRESHDESK_API_KEY
            or FRESHDESK_API_KEY in demo_placeholder_values
        ):
            errors.append(
                "Missing required setting: FRESHDESK_API_KEY "
                "(Freshdesk API key)"
            )

        if (
            not ANTHROPIC_API_KEY
            or ANTHROPIC_API_KEY in demo_placeholder_values
        ):
            errors.append(
                "Missing required setting: ANTHROPIC_API_KEY "
                "(Anthropic API key)"
            )
    else:
        warnings.append(
            "Running in DEMO MODE — "
            "Freshdesk and Anthropic API keys not required."
        )

    if EMAIL_NOTIFICATIONS_ENABLED and not DEMO_MODE:
        if not SMTP_EMAIL:
            warnings.append(
                "SMTP_EMAIL not set — "
...
def get_all_settings() -> dict:
    """
    Return all current settings as a dict.
    Useful for logging active config at startup
    and for the dashboard settings page.
    Masks sensitive values like API keys as '***'.
...
def setup_logging() -> None:
    """
    Configure the Python logging system using settings above.
    Called once from main.py at startup.
    Creates rotating file handler and console handler.
    Silences noisy third-party loggers.
    Safe to call multiple times — clears handlers first.
    """
    from logging.handlers import RotatingFileHandler
...
if __name__ == "__main__":
    ...
    print(
        f"  Demo mode    : "
        f"{'YES — placeholder keys accepted' if DEMO_MODE else 'NO — live mode'}"
    )
    ...
    print("\n--- Category info ---")
    print(
        f"  Valid categories       : {len(VALID_CATEGORIES)}"
    )
    print(
        f"  Auto-resolvable        : "
        f"{len(AUTO_RESOLVABLE_CATEGORIES)}"
    )
    print(
        f"  Manual-only            : "
        f"{len(MANUAL_ONLY_CATEGORIES)}"
    )
    print(
        f"  Force escalation words : "
        f"{len(FORCE_ESCALATION_KEYWORDS)}"
    )
    ...
    print(
        f"  Claude API     : "
        f"{RATE_LIMIT_CLAUDE_RPM} req/min"
    )
    print(
        f"  Freshdesk API  : "
        f"{RATE_LIMIT_FRESHDESK_RPM} req/min"
    )
    print(
        f"  SMTP emails    : "
        f"{RATE_LIMIT_SMTP_PER_HOUR} per hour"
    )
    print(
        f"  Intune API     : "
        f"{RATE_LIMIT_INTUNE_RPM} req/min"
    )

    print("\n--- Automation script map ---")
    for cat, script in AUTOMATION_SCRIPT_MAP.items():
        script_label = script or "— (manual only)"
        print(f"  {cat:<20} → {script_label}")
