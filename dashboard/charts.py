...
def _now_utc() -> datetime:
    ...
    Uses timezone-aware object to avoid deprecation warning.
...
def daily_trend_chart(
    ...
    Args:
        daily_rows : List of dicts from get_daily_stats()
                     Each dict has: date, total, resolved, escalated
        days       : Number of days shown (used for empty state)
    ...
    if not daily_rows:
        return _empty_chart(
            f"No ticket data for last {days} days.\n"
            "Run python main.py to process tickets."
        )
...
def category_donut_chart(category_counts: dict) -> go.Figure:
    ...
    Args:
        category_counts : Dict of category -> count
                          from get_resolution_stats()

    Returns:
        Plotly Figure object
    """
    if not category_counts:
        return _empty_chart("No category data available yet.")
...
def resolution_bar_chart(logs: list) -> go.Figure:
    """
    Horizontal bar chart showing how many tickets were
    resolved by each method — AI auto, KB guide, escalation.
    Includes all resolver types from RESOLVER_COLORS.

    Args:
        logs : List of ticket log dicts from get_all_ticket_logs()

    Returns:
        Plotly Figure object
    """
    if not logs:
        return _empty_chart("No resolution data available yet.")
    ...
    color_map = {
        ...
        "Escalated to Engineer": COLORS["amber"],
        "Engineer Queue"       : COLORS["coral"],
        "Force Escalated"      : COLORS["red"],
        "Emergency Escalation" : COLORS["pink"],
    }
...
def priority_distribution_chart(logs: list) -> go.Figure:
    """
    Grouped bar chart showing priority distribution
    (low/medium/high/urgent) across ticket categories.
    ...
    """
    if not logs:
        return _empty_chart("No priority data available yet.")
    ...
    if (
        ...
        return _empty_chart(
            "Missing category or priority columns."
        )
...
def auto_resolve_gauge(auto_rate_pct: float) -> go.Figure:
    """
    Gauge chart showing the overall AI auto-resolution rate.
    Green zone = good (>60%), amber = average (30-60%),
    red = low (<30%). Threshold line at 60%.
...
def hourly_ticket_heatmap(logs: list) -> go.Figure:
    """
    Heatmap showing which hours of the day and days of the
    week receive the most tickets. Useful for staffing.
    Uses timezone-aware UTC datetime parsing.
    ...
    if df.empty:
        return _empty_chart("No valid timestamps in logs.")
...
def resolution_time_histogram(logs: list) -> go.Figure:
    """
    Histogram showing distribution of ticket processing times
    in minutes. Helps identify backlogs and SLA compliance.
    Uses timezone-aware UTC datetime parsing.
    ...
    """
    if not logs:
        return _empty_chart("No data available for histogram.")

    df = pd.DataFrame(logs)

    if (
        "created_at" not in df.columns
        or "updated_at" not in df.columns
    ):
        return _empty_chart("Missing timestamp columns.")
...
def category_trend_chart(
    logs: list,
    days: int = 14,
) -> go.Figure:
    """
    Multi-line chart showing how each ticket category trends
    over the last N days. Shows top 5 categories by volume.
    Uses timezone-aware UTC datetime for cutoff calculation.
    ...
    if df.empty:
        return _empty_chart(
            f"No ticket data in the last {days} days."
        )
...
def automation_success_chart(
    automation_logs: list,
) -> go.Figure:
    """
    Grouped bar chart showing automation script success vs
    failure count for each script type.
    Script names have .ps1 stripped for readability.
...
def summary_table_chart(stats: dict) -> go.Figure:
    ...
    Table visualization of overall system performance stats.
    Good for a quick at-a-glance summary card.
    Includes KB guide count from updated stats dict.

    Args:
        stats : Dict from get_resolution_stats()
    ...
    rows = [
        ...
        ["AI Auto-Resolved",        str(stats.get("ai_resolved",   0))],
        ["Escalated to Engineer",   str(stats.get("escalated",     0))],
        ["KB Guides Sent",          str(stats.get("kb_sent",       0))],
        ["Auto-Resolution Rate",    f"{stats.get('auto_rate_pct', 0.0):.1f}%"],
    ...
    fig = go.Figure(data=[go.Table(
        ...
        cells = dict(
            values     = list(zip(*rows)),
            fill_color = [
                ["#f9f9f9", "white"] * (len(rows) + 1)
            ],
            font       = dict(
                color=COLORS["dark"], size=13
            ),
            align      = "left",
            height     = 32,
        ),
...
def _empty_chart(message: str) -> go.Figure:
    """
    Return a blank chart with a centered message.
    Used as fallback when there is no data to display.
...
if __name__ == "__main__":
    ...
    print(
        f"  UTC now : "
        f"{_now_utc().strftime('%d %b %Y %H:%M UTC')}"
    )
    print()
    ...
    sample_tickets = [
        ("app_install",    "high",   "Zoom installed.",          "AI_AUTO",          "RESOLVED"),
        ("app_install",    "medium", "Chrome installed.",         "AI_AUTO",          "RESOLVED"),
        ("antivirus",      "medium", "AV updated.",              "AI_AUTO",          "RESOLVED"),
        ("antivirus",      "high",   "AV scan triggered.",       "AI_AUTO",          "RESOLVED"),
        ("password_reset", "high",   "AD password reset.",       "AI_AUTO",          "RESOLVED"),
        ("network",        "high",   "VPN guide sent.",          "KB+ESCALATION",    "ESCALATED"),
        ("hardware",       "high",   "Onsite scheduled.",        "ESCALATION",       "ESCALATED"),
        ("email_issue",    "medium", "Outlook repair.",          "ESCALATION",       "ESCALATED"),
        ("printer",        "low",    "Spooler restarted.",       "AI_AUTO",          "RESOLVED"),
        ("os_issue",       "medium", "SFC and DISM ran.",        "AI_AUTO",          "RESOLVED"),
        ("access_permission","high", "Permissions updated.",     "ESCALATION",       "ESCALATED"),
        ("other",          "low",    "Assigned to engineer.",    "ENGINEER_QUEUE",   "ESCALATED"),
    ]

    print("Inserting sample data...")
    for i, (cat, pri, action, resolver, status) in enumerate(
        sample_tickets, start=4001
    ):
        log_ticket_action(
            ticket_id    = i,
            category     = cat,
    ...
    log_automation(4001, "install_app.ps1",          "PC-0042",    True,  "Zoom installed.",    "",                  42.1)
    log_automation(4002, "install_app.ps1",          "PC-0043",    True,  "Chrome installed.",  "",                  38.5)
    log_automation(4003, "update_antivirus.ps1",     "LAPTOP-115", True,  "AV updated.",        "",                  65.2)
    log_automation(4004, "update_antivirus.ps1",     "LAPTOP-116", False, "",                   "Connection refused",10.0)
    log_automation(4005, "reset_password.ps1",       "PC-0099",    True,  "Password reset.",    "",                  12.3)
    log_automation(4009, "restart_print_spooler.ps1","PC-0099",    True,  "Spooler restarted.", "",                   8.7)
    log_automation(4010, "clear_disk_space.ps1",     "WS-202",     True,  "SFC complete.",      "",                 180.4)

    stats           = get_resolution_stats()
    all_logs        = get_all_ticket_logs(limit=200)
    daily_rows      = get_daily_stats(days=7)
    automation_logs = get_recent_automation_logs(limit=50)
    ...
    charts = [
        ...
        ("Category Donut Chart",        category_donut_chart(stats["category_counts"])),
        ("Resolution Bar Chart",        resolution_bar_chart(all_logs)),
        ("Priority Distribution",       priority_distribution_chart(all_logs)),
        ("Auto-Resolve Gauge",          auto_resolve_gauge(stats["auto_rate_pct"])),
        ("Hourly Heatmap",              hourly_ticket_heatmap(all_logs)),
        ("Resolution Time Histogram",   resolution_time_histogram(all_logs)),
        ("Category Trend Chart",        category_trend_chart(all_logs, days=14)),
        ("Automation Success Chart",    automation_success_chart(automation_logs)),
        ("Summary Table",               summary_table_chart(stats)),
        ("Empty Chart (test)",          _empty_chart("Test empty state")),
    ]
    ...
    for name, fig in charts:
        trace_count = len(fig.data)
        has_data    = trace_count > 0
        status      = "OK ✓" if has_data else "Empty (no data)"
        print(
            f"  {name:<35} "
            f"{trace_count:<8} "
            f"{status}"
    ...
    print(f"\n  KB guides sent  : {stats.get('kb_sent', 0)}")
    print(f"  Force escalated : {stats.get('force_escalated', 0)}")
    print(f"  Auto-rate       : {stats.get('auto_rate_pct', 0)}%")
