...
def _now_str() -> str:
    ...
    Returns:
        Formatted datetime string e.g. '2026-05-20 10:30:00'
    """
...
def build_index(force_rebuild: bool = False) -> bool:
    ...
    log.info("=" * 55)
    log.info(f"{mode}KB INDEXER — Starting build")
    log.info(f"  Docs dir    : {DOCS_DIR}")
    log.info(f"  Chroma dir  : {CHROMA_DIR}")
    log.info(f"  Embed model : {EMBED_MODEL}")
    log.info(f"  Chunk size  : {CHUNK_SIZE} words")
    log.info(f"  Overlap     : {CHUNK_OVERLAP} words")
    log.info("=" * 55)
    ...
    if not doc_files:
        log.warning(
            f"No documents found in '{DOCS_DIR}'. "
            "Creating sample docs and indexing..."
        )
        create_sample_docs()
        doc_files = _scan_docs_folder()

        if not doc_files:
            log.error("Still no documents after creating samples.")
            return False

    log.info(
        f"Found {len(doc_files)} document(s) in docs folder."
    )
    ...
    try:
        ...
        try:
            ...
            log.info(
                f"Created new ChromaDB collection "
                f"'{COLLECTION_NAME}'."
            )
    ...
    log.info("=" * 55)
    log.info(
        f"{mode}KB INDEX COMPLETE — "
        f"{indexed_files} file(s), "
        f"{total_chunks} new chunk(s)."
    )
    log.info(
    ...
    return indexed_files > 0 or total_chunks == 0


def _scan_docs_folder() -> list:
    """
    Scan the docs/ directory and return all supported files
    ...
    if not docs_path.exists():
        log.error(
            f"Docs folder does not exist: {DOCS_DIR}"
        )
        return []
...
def _load_document(file_path: Path) -> str:
    ...
    Args:
        file_path : Path object pointing to the document
    ...
    try:
        ...
        try:
            with open(
                ...
                text = f.read().strip()
            log.warning(
                f"File '{file_path.name}' read with "
                "latin-1 encoding "
                "(consider saving as UTF-8)."
            )
            return text
...
def _chunk_text(text: str, file_name: str) -> list:
    ...
    Chunking strategy:
        ...
        2. If a paragraph exceeds CHUNK_SIZE words,
           split further by sentences
        3. Merge small adjacent chunks (< 30 words)
        4. Add CHUNK_OVERLAP words of context from the
           previous chunk to the start of each new chunk

    Args:
        text      : Full document text to split
        file_name : Document filename for chunk metadata
    ...
    for para in paragraphs:
        ...
        if len(words) <= CHUNK_SIZE:
            ...
            for sentence in sentences:
                ...
                if (
                    word_count + len(sentence_words) > CHUNK_SIZE
                    and current
                ):
                    raw_chunks.append(" ".join(current))
                    overlap_words = (
    ...
    for idx, chunk_text in enumerate(merged_chunks):
        if prev_words and CHUNK_OVERLAP > 0:
            overlap    = " ".join(
                prev_words[-CHUNK_OVERLAP:]
            )
            chunk_text = f"{overlap} {chunk_text}"

        chunk_id = _generate_chunk_id(
            file_name, idx, chunk_text
        )
...
def _split_into_sentences(text: str) -> list:
    """
    Split a paragraph into individual sentences.
    Protects common abbreviations so "Dr." and "e.g."
    do not cause false sentence splits.
...
def _merge_small_chunks(
    ...
    """
    Merge chunks that are too small (under min_words words)
    with the adjacent chunk so every chunk has enough context.

    Args:
        chunks    : List of raw chunk text strings
        min_words : Minimum word count — merge if below this
...
def _add_chunks_to_collection(
    ...
    """
    Add text chunks to the ChromaDB collection in batches of 50.
    Timestamps use timezone-aware UTC to avoid deprecation warning.
...
def _remove_existing_chunks(
    collection,
    file_name: str,
) -> None:
    """
    Delete all existing chunks from a specific file before
    re-indexing. Prevents duplicate entries when a file is
    updated and re-indexed.

    Args:
        collection : ChromaDB collection object
        file_name  : Source document filename to remove
    """
    try:
        existing = collection.get(
            where={"file_name": file_name}
...
def _get_changed_files(
    ...
    """
    Compare current file hashes against stored metadata to
    find files that are new or have been modified since the
    last index run.
    ...
    Returns:
        List of Path objects that need re-indexing
    """
    changed = []
    ...
    for file_path in doc_files:
        ...
        if current_hash != stored_hash:
            ...
            log.debug(
                f"  {file_name} — unchanged (skipping)"
            )
...
def _hash_file(file_path: Path) -> str:
    ...
    Returns:
        MD5 hex digest string or empty string on failure
    """
    try:
        hasher = hashlib.md5()
        with open(file_path, "rb") as f:
        ...
        log.error(
            f"Cannot hash file '{file_path.name}': {e}"
        )
        return ""
...
def _generate_chunk_id(
    ...
    """
    Generate a stable unique ID for a chunk.
    Based on file name + chunk index + content hash so the
    same chunk always gets the same ID across index runs.
    No spaces in the ID.
    ...
    Returns:
        Unique chunk ID string with no spaces
    """
    content    = f"{file_name}:{chunk_index}:{chunk_text[:100]}"
...
def _load_metadata() -> dict:
    """
    Load the index metadata JSON file that tracks which files
    have been indexed and their content hashes.
...
def _save_metadata(metadata: dict) -> None:
    ...
    """
    try:
        meta_dir = os.path.dirname(METADATA_FILE)
        if meta_dir:
...
def get_index_stats() -> dict:
    """
    Return statistics about the current ChromaDB index.
    Safe to call when chromadb is not yet installed — returns
    an error key in that case.

    Returns:
        Dict with: total_chunks, collection_name,
                   indexed_files, chroma_dir, docs_dir,
                   metadata, embed_model
    """
    try:
        import chromadb
        from chromadb.utils import embedding_functions
...
def delete_index() -> bool:
    """
    Completely delete the ChromaDB index and metadata file.
    Use to do a clean rebuild from scratch.
    WARNING — all indexed data will be lost.
    ...
    try:
        if os.path.exists(CHROMA_DIR):
            shutil.rmtree(CHROMA_DIR)
            log.info(
                f"Deleted ChromaDB directory: {CHROMA_DIR}"
            )

        if os.path.exists(METADATA_FILE):
            os.remove(METADATA_FILE)
            log.info(
                f"Deleted metadata file: {METADATA_FILE}"
            )

        log.info(
            "Index deleted. Run build_index() to rebuild."
        )
        return True
...
def create_sample_docs() -> None:
    """
    Create sample IT support guide documents in docs/ folder.
    Run once to populate the knowledge base with starter content.
    Skips files that already exist so it is safe to call repeatedly.
    """
    os.makedirs(DOCS_DIR, exist_ok=True)
...
This guide covers common printer issues on ICICI Bank corporate network printers.
...
Step 6: If still offline, right click the printer and Set as default printer.
Step 7: Restart the Print Spooler service:
Open Run dialog with Windows plus R. Type services.msc and press Enter.
...
Step 3: Select all jobs with Ctrl plus A.
Step 4: Click Document menu and select Cancel.
Step 5: If jobs cannot be cancelled, stop the Print Spooler service.
Open File Explorer and go to C:\\Windows\\System32\\spool\\PRINTERS.
Delete all files in this folder but not the folder itself.
Start the Print Spooler service again and try printing.
...
This guide covers common Outlook and email issues on ICICI Bank corporate laptops.
...
Step 4: If Outlook does not open in Safe Mode, repair Office:
Go to Control Panel, Programs, Programs and Features.
Right click Microsoft Office and select Change.
Select Quick Repair and click Repair.
...
Step 2: Look at the status bar at the bottom of Outlook.
It should say Connected to Microsoft Exchange.
If it says Disconnected or Working Offline, click Send/Receive and uncheck Work Offline.
...
""",
        ...
        "windows_os_guide.txt": """
Windows OS Troubleshooting Guide

This guide covers common Windows operating system issues on ICICI Bank corporate machines.

Issue: Windows Update Failing or Stuck

Step 1: Open Settings, go to Update and Security, click Windows Update.
Step 2: Click Check for updates.
Step 3: If an update is stuck, restart your computer and try again.
Step 4: If updates keep failing, clear the Windows Update cache:
Open Command Prompt as Administrator.
Type net stop wuauserv and press Enter.
Type net stop bits and press Enter.
Open File Explorer and go to C:\\Windows\\SoftwareDistribution.
Delete all contents inside the SoftwareDistribution folder.
Type net start wuauserv to restart the service.
Try running Windows Update again.

Issue: Blue Screen of Death (BSOD)

A blue screen crash means Windows encountered a critical error.
Note the error code shown on the blue screen if possible.
Contact IT support with the error code and your machine name.
Our team will check Windows Event Viewer and crash dump files remotely.
Common causes: faulty driver, RAM issue, corrupted system files.

Issue: Computer Running Slow

Step 1: Press Ctrl plus Shift plus Escape to open Task Manager.
Check which processes are using high CPU or memory.
Step 2: Open Settings, go to Apps, Startup and disable unnecessary startup programs.
Step 3: Run Disk Cleanup: search for Disk Cleanup in Start menu.
Select C: drive, check all boxes, click OK, then Clean up system files.
Step 4: Run system file checker: open Command Prompt as Administrator.
Type sfc /scannow and press Enter. Wait for it to complete.
Restart your computer after the scan finishes.

Issue: Disk Space Full

Step 1: Open File Explorer, right click C: drive and select Properties.
Click Disk Cleanup to free up temporary files.
Step 2: Uninstall unused applications via Control Panel, Programs.
Step 3: Move large files like videos and downloads to a network share or external drive.
Step 4: If disk is still full after cleanup, contact IT support.
We will check for oversized log files and temporary data.

Issue: Computer Not Starting or Black Screen

Step 1: Hold the power button for 10 seconds to force shutdown.
Step 2: Wait 30 seconds and power on again.
Step 3: If you see a spinning circle, Windows is loading — wait up to 5 minutes.
Step 4: If screen stays black, try pressing F8 during startup for recovery options.
Step 5: Contact IT support if the machine still does not boot.
Do not attempt to reinstall Windows yourself.
""",
    }
    ...
    for filename, content in docs.items():
        ...
        if not os.path.exists(file_path):
            ...
            log.debug(
                f"Sample doc already exists: {filename}"
            )
            skipped_count += 1

    log.info(
        f"Sample docs complete — "
        f"{created_count} created, "
        f"{skipped_count} already existed. "
        f"Run build_index() to index them."
    )
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print(
            f"  Embed model  : {stats.get('embed_model', EMBED_MODEL)}"
        )
        print(
            f"  Chroma dir   : {stats['chroma_dir']}"
        )
        ...
        for fname, meta in stats.get("metadata", {}).items():
            print(
                f"  {fname:<40} "
                f"{meta['chunk_count']:<8} "
                f"{meta['indexed_at']}"
            )

        print(
            "\nStep 4: Testing incremental update "
            "(no changes expected)..."
        )
        success2 = build_index(force_rebuild=False)
        print(
        ...
        print(
            f"  Force rebuild: "
            f"{'SUCCESS ✓' if success3 else 'FAILED ✗'}"
        )

        final_stats = get_index_stats()
        print(
            f"\n  Final chunk count : "
            f"{final_stats['total_chunks']}"
        )

    else:
        print(
            "\nIndexing failed — check logs above for errors.\n"
            "Tip: Make sure chromadb is installed:\n"
            "  pip install chromadb sentence-transformers "
            "--break-system-packages"
        )

    print("\n" + "=" * 60)
    print("KB Indexer test complete.")
    print(
        "Next step: Run "
        "knowledge_base/kb_search.py to test searching."
    )
    print("=" * 60 + "\n")