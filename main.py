...
def process_tickets() -> None:
    ...
    Full flow per ticket:
        ...
        7.  If can auto-resolve → run automation via orchestrator
            a. Success  → close + notify user
            b. Failure  → search KB → send guide or escalate
        8.  If cannot auto_resolve → search KB + escalate
        9.  Log everything to database
    """
    global TICKETS_PROCESSED, TICKETS_RESOLVED, TICKETS_ESCALATED, POLL_COUNT
    ...
    if not new_tickets:
        log.info(
            "All fetched tickets already processed this session."
        )
        return
...
def _process_single_ticket(
    ...
    try:
        ...
        try:
            ...
            log.error(f"│  Rule classifier also failed: {e2}")
            _emergency_escalate(
                ticket, "Both classifiers failed."
            )
            TICKETS_ESCALATED += 1
            return
    ...
    if machine == "UNKNOWN" and can_auto:
        can_auto = False
        log.warning("│  Machine UNKNOWN — auto-resolve disabled.")
    ...
    if force_escalate:
        log.warning(
            "│  FORCE ESCALATION — critical keyword detected!"
        )
        _handle_force_escalation(ticket, classification)
        TICKETS_ESCALATED += 1
        log.info(f"└─ Ticket #{ticket_id} → FORCE ESCALATED")
    ...
    if after_hours and priority not in ["high", "urgent"]:
        log.info(
            "│  After hours + non-critical → "
            "after-hours escalation."
        )
        escalate_after_business_hours(ticket, classification)
        TICKETS_ESCALATED += 1
        log.info(
            f"└─ Ticket #{ticket_id} → AFTER HOURS ESCALATED"
        )
        return

    if priority in ["high", "urgent"] and not can_auto:
        log.warning(
            "│  High priority + no auto-resolve → "
            "urgent escalation."
        )
        escalate_high_priority(ticket, classification)
        TICKETS_ESCALATED += 1
        log.info(
            f"└─ Ticket #{ticket_id} → HIGH PRIORITY ESCALATED"
        )
        return
...
def _handle_auto_resolve(
    ...
    log.info("│  Attempting auto-resolution...")
    ...
    if success:
        log.info("│  Auto-resolution SUCCEEDED ✓")
        ...
        resolution_msg = (
            ...
            f"Action taken : {suggested_action}\n\n"
            f"Your issue has been fixed remotely. "
            f"Please restart your computer if needed and "
            f"verify the issue is resolved.\n\n"
            f"If the problem persists, please raise a new ticket.\n\n"
            f"Thank you,\n"
            f"IT Support Team\n"
            f"{COMPANY_NAME}"
        ...
        log.warning(
            "│  Auto-resolution FAILED — checking KB..."
        )
...
def _handle_manual_route(
    ...
    log.info("│  Cannot auto-resolve — searching KB...")
...
def _search_kb_and_escalate(
    ...
    if kb_guide:
        log.info(
            f"│  KB guide found — sending to {requester}."
        )
...
def _handle_force_escalation(
    ...
    log_ticket_action(
        ...
        priority     = "urgent",
        action_taken = (
            "Force-escalated — critical keyword detected."
        ),
        resolved_by  = "FORCE_ESCALATION",
        status       = "ESCALATED",
    )
...
def _startup_checks() -> bool:
    ...
    if not validation["valid"]:
        for err in validation["errors"]:
            log.error(f"  CONFIG ERROR   : {err}")
        log.error(
            "Fix the errors above in config/.env and restart."
        )
        return False
    for warn in validation["warnings"]:
        log.warning(f"  CONFIG WARNING : {warn}")
...
def main() -> None:
    ...
    DEMO_MODE=true  → Runs full demo with 10 sample ICICI tickets.
                      No real API keys needed. Everything simulated.

    DEMO_MODE=false → Runs in live mode with real Freshdesk + Claude.
                      Polls Freshdesk every N minutes.
    """
    signal.signal(signal.SIGINT,  _signal_handler)
    signal.signal(signal.SIGTERM, _signal_handler)
